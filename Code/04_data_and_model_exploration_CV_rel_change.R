## data and model exploration for canopy volume - RELATIVE CHANGE
...
#   mutate(lag.CV = dplyr::lag(CV, n = 1, default = NA)) %>% ## lag CV one season
#   #mutate(CVLag = c(CV[1], CV[-793])) %>%
#   # mutate(Year = as.numeric(Year)) %>%
#   mutate(CV.relative = CV/lag.CV) %>% ## relative CV 
#   mutate(CV.difference = CV-lag.CV) %>%## delta CV i.e., difference between
#   ungroup()
# names(bioMeanQ_long_distRel)

## relative change without restarting when damaged
bioMeanQ_long_distRel2 <- damSeq %>%
  drop_na(CV) %>%
  ...
  mutate(CV.difference = CV-lag.CV) %>%## delta CV i.e., difference between
  ungroup()
names(bioMeanQ_long_distRel2)
## join back to main df

bioMeanQ_long_distJoin <- full_join(bioMeanQ_long_distRel2, bioMeanQ_long_dist, by = c("PlantID","Species", "Season", "Group", "LifeForm", "Year", "CV", "Replacement", "Damage"))

# CV model with damaged tree variable ---------------------------------------------------------------
...
rf.data <- bioMeanQ_long_distJoin %>%
  dplyr::select(CV.relative, CV, PlantID, Species:LifeForm, Month, Year, Season, POS, Substrate,
  ...
  # drop_na(CV.relative) %>%
  mutate(CV.relative = ifelse(is.na(lag.CV), 1, CV.relative)) %>%
  select(-Source, -lag.CV)
...
plantids <- as.data.frame(rf.data$PlantID) %>%
  rename(PlantID = 1)
plantids$CV <- rf.data$CV

## remove plantid from df

rf.data <- rf.data %>% select(-PlantID, -CV)
...
rf.data.imputed <- rfImpute(CV.relative ~ ., rf.data)
...
rf.data.val <- rf.data.imputed %>%
  rename(y = CV.relative) %>% as.data.frame() %>%
...
pdf(paste0( "Figures/01_var_imp_full_model_CV_Relative.pdf"), width=12, height=8)
...
rf.data.red <- rf.data.imputed %>%
  select(CV.relative, all_of(vars), -LifeForm) %>%
  mutate(Species = as.factor(Species), Group = as.factor(Group), Year = as.factor(Year)) %>% 
  droplevels()
...
mean(rf$rsq) ## 80
rf
save(rf, file = "models/04_CV_final_rf.RData")
...
file.name1 <- "Figures/04_relative_importance_CV_relative_inc_damage.jpg"
...
jpeg(paste0( "Figures/04_CV_relative_Season.jpg"))
partialPlot(rf, rf.data.val, Season)
dev.off()

jpeg(paste0( "Figures/04_CV_relative_Distance.jpg"))
partialPlot(rf, rf.data.val, DistToSJC002)
dev.off()

jpeg(paste0( "Figures/04_CV_relative_Q.jpg"))
partialPlot(rf, rf.data.val, Q)
dev.off()
jpeg(paste0( "Figures/04_CV_relative_Species.jpg"))
partialPlot(rf, rf.data.val, Species)
dev.off()
jpeg(paste0( "Figures/04_CV_relative_SJC.jpg"))
partialPlot(rf, rf.data.val, SJC_002)
dev.off()
jpeg(paste0( "Figures/04_CV_relative_Replacement.jpg"))
partialPlot(rf, rf.data.val, Replacement)
dev.off()
jpeg(paste0( "Figures/04_CV_relative_Group.jpg"))
partialPlot(rf, rf.data.val, Group)
dev.off()

jpeg(paste0( "Figures/04_CV_relative_POS.jpg"))
partialPlot(rf, rf.data.val, POS)
dev.off()

jpeg(paste0( "Figures/04_CV_relative_Substrate.jpg"))
partialPlot(rf, rf.data.val, Substrate)
dev.off()

jpeg(paste0( "Figures/04_CV_relative_Year.jpg"))
partialPlot(rf, rf.data.val, Year)
dev.off()

jpeg(paste0( "Figures/04_CV_relative_Damage.jpg"))
...
## predict change in CV in response to reduction in discharge
...
## join CV to run model on real values
...
dim(rf.data.val)
## run reduced model
rfOF <- randomForest(CV~., data=rf.data.valRaw, importance = T, ntree = 1000)
mean(rfOF$rsq) ## 84% accuracy
rfOF
...
for(c in 1:length(percentChange)) {
  ...
  predCV <- as.data.frame(predict(rfOF, newdata = rf.data.valNew)) %>%
    rename(CV = 1) 
  
  head(predCV)
  
  ## add new data to predicted values
  pred_df <- cbind(predCV, rf.data.valNew, rf.data[, "Month"])  %>%
...
## get mean CV per plant id 

pred_dfx1 <- pred_dfx %>%
  group_by(PlantID, Season, PercentReduction) %>%
  mutate(CVMean = median(CV)) %>%
  mutate(PercentReduction = as.factor(PercentReduction))

## save out 

save(pred_dfx1, file = "ignore/04_CV_reduction_predictions.RData")
...
pred_red <- pred_dfx %>%
  select(CV,  PlantID, Season, Species, Group, PercentReduction,  Year) %>%
...
p1 <- ggplot(data = pred_red, aes(x = PercentReduction, y = CV, group = Species, color = Species)) +
  stat_smooth() +
  facet_wrap(~Group)

p1

file.name1 <- "Figures/04_CV_predictions_reduction_outfallQ.jpg"
...
baseline <- pred_dfx2 %>%
  ...
  select(CV, PlantID, Year, Season, Species)

## get CV after 2020 reduction
pred <- pred_dfx2 %>%
  filter(PercentReductionScenario == percentChangeScen[20], Year %in% c(2018, 2019, 2020, 2021, 2022)) %>%
  select(CV, PlantID, Year, Season) %>%
  rename(PredCV = CV)

dat <- cbind(baseline, pred[,"PredCV"]) %>%
  rename(PredCV = 6)
...
ttest <- t.test(datSP$CV, datSP$PredCV, paired =T)
## no significance
ttest
...
for(t in 1:length(percentChangeScen)) {
  ...
  pred <- pred_dfx2 %>%
    ...
    select(CV, PlantID, Year, Season, PercentReduction) %>%
    rename(PredCV = CV)
  ...
  ## paired t test
  ttest <- t.test(dat$CV, dat$PredCV, paired =T)
  ...
  df[t,5] <- length(na.omit(dat$CV))
  df[t,6] <- length(na.omit(dat$PredCV))
  df[t,7] <- mean(na.omit(dat$CV))
  df[t,8] <- mean(na.omit(dat$PredCV))
...
p2 <- ggplot(data = df, aes(x = Reduction, y = PValueR)) +
  stat_smooth() +
  geom_point() +
  scale_y_continuous(name = "P Value") +
  scale_x_continuous(name = "Reduction in Combined Outfall Discharge")

p2

file.name1 <- "Figures/04_CV_pvals_reduction_outfallQ.jpg"
ggsave(p2, filename=file.name1, dpi=300, height=5, width=8)

## 31% reduction is where CV changes significantly
## baseline - 85.23819
## predicted - 85.21370
...
pred <- pred_dfx2 %>%
  ...
  select(CV, PlantID, Year, Season, PercentReduction, SJC002_POM001Combined) %>%
  rename(PredCV = CV)
...
corrf <- rf.data.val[,c(10:16)]
cor(corrf)
## rescale variables 

## rescale variables
rf.data.rescale <- rf.data.valRaw %>%
  mutate(
    # Q_sc = rescale(Q, to = c(0,1)),
...
## all combined
mod1 <- lmer(formula = CV ~ Replacement + Species + Group +  Season +
               DistToSJC002_sc+ Substrate+ POS+ DischargeSJC002_POM001_WN001Combined_sc + Damage2+
...
icc(mod1)
r2_nakagawa(mod1) ## 0.177

## pom and sjc combined
mod2 <- lmer(formula = CV.relative ~  Replacement + Species + Group +  Season +
...
icc(mod2)
r2_nakagawa(mod2) ## 0.035

## individual outfall discharge

mod3 <- lmer(formula = CV.relative ~  Replacement + Species + Group +  Season +
               DistToSJC002_sc+ Substrate+ SJC002_sc + POM001_sc + Damage2+
               # (1|Year) + ## remove due to low ICC
               # (1|Group) + ## remove due to low ICC
               # (1|Year) +
               (1|Year),
...
ests
file.name1 <- paste0(out.dir, "effect_sizes_drivers_of_CV_relative_inc_damage.jpg")
...
out.filename <- paste0(out.dir,"00_mixed_mod_CV_relative_random_effects_inc_damage.jpg")
...
out.filename <- paste0(out.dir,"00_mixed_mod_CV_relationships_combined_outfall.jpg")
