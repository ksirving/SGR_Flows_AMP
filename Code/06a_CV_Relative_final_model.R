...
sum(bioMeanQ_long_dist$Damage == "Y") ## 506a_relative
...
bioMeanQ_long_distJoin <- full_join(bioMeanQ_long_distRel, bioMeanQ_long_dist, by = c("PlantID","Species", "Season", "Group", "LifeForm", "Year", "CV", "Replacement", "Damage"))

# CV model on relative change---------------------------------------------------------------
...
## relative per tree
rf.data.imputedRel <- rfImpute(CV.relative ~ ., rf.data)
...
mean(rf$rsq) ##  80
rf
save(rf, file = "models/06a_CV_relative_final_rf.RData")
...
file.name1 <- "Figures/06a_relative_importance_CV_relative_final_model.jpg"
ggsave(i1, filename=file.name1, dpi=300, height=5, width=8)


# Partial Plots -----------------------------------------------------------

jpeg(paste0( "Figures/06a_relative_CV_relative_Season.jpg"))
partialPlot(rf, rf.data.val, Season)
dev.off()

jpeg(paste0( "Figures/06a_relative_CV_relative_Distance.jpg"))
partialPlot(rf, rf.data.val, DistToSJC002)
dev.off()

jpeg(paste0( "Figures/06a_relative_CV_relative_Q.jpg"))
partialPlot(rf, rf.data.val, Q)
dev.off()

jpeg(paste0( "Figures/06a_relative_CV_relative_Species.jpg"))
partialPlot(rf, rf.data.val, Species)
dev.off()

jpeg(paste0( "Figures/06a_relative_CV_relative_SJC.jpg"))
partialPlot(rf, rf.data.val, SJC_002)
dev.off()

jpeg(paste0( "Figures/06a_relative_CV_relative_Replacement.jpg"))
partialPlot(rf, rf.data.val, Replacement)
dev.off()

jpeg(paste0( "Figures/06a_relative_CV_relative_Group.jpg"))
partialPlot(rf, rf.data.val, Group)
dev.off()

jpeg(paste0( "Figures/06a_relative_CV_relative_POS.jpg"))
partialPlot(rf, rf.data.val, POS)
dev.off()

jpeg(paste0( "Figures/06a_relative_CV_relative_Substrate.jpg"))
partialPlot(rf, rf.data.val, Substrate)
dev.off()

jpeg(paste0( "Figures/06a_relative_CV_relative_Year.jpg"))
partialPlot(rf, rf.data.val, Year)
dev.off()

jpeg(paste0( "Figures/06a_relative_CV_relative_Damage.jpg"))
...
dataall <- cbind(plantids, rf.data.imputedRel) %>%
  drop_na(CV) %>%
  select(PlantID, CV, names(rf.data.red))
...
mod1 <- lmer(formula = CV.relative ~  Replacement + Species + Group + 
...
icc(mod1)
r2_nakagawa(mod1) ## 0.044


## pom and sjc combined
mod2 <- lmer(formula = CV.relative ~ Replacement + Species + Group + 
               DistToSJC002_sc+ Substrate+ POS+ SJC002_POM001Combined_sc + Damage2+
               # (1|Year) + ## remove due to low ICC
               # (1|Group) + ## remove due to low ICC
               (1|Year) +
...
icc(mod2)
r2_nakagawa(mod2) ## 0.004

## individual outfall discharge

mod3 <- lmer(formula = CV.relative ~ Replacement + Species + Group + 
...
(rf.CV <- rf.crossValidation(rf, rf.data.val[, 2:13], ## check indexing!!!!
