...
rf.data <- bioMeanQ_long_distJoin %>%
  dplyr::select(SWP.difference, Species:LifeForm, Month, Year, Season, POS, Substrate,
...
rf.data.imputed <- rfImpute(SWP.difference ~ ., rf.data)
...
rf
mean(rf$rsq) ## seasonal - 0.70, survey - 0.79 ,damage = 82.7, difference = 71
...
pdf(paste0( "Figures/01_var_imp_full_model_SWP_difference.pdf"), width=12, height=8)
...
rf.data.red <- rf.data.imputed %>%
  select(SWP.difference, all_of(vars), -LifeForm) %>%
  mutate(Species = as.factor(Species), Group = as.factor(Group), Year = as.factor(Year)) %>% 
  droplevels()
...
mean(rf$rsq) ## season - 0.75, survey - 0.85, damage = 84.2, difference = 71
rf
...
## get mean and scale and % to make difference
...
file.name1 <- "Figures/04_relative_importance_SWP_difference_inc_damage.jpg"
...
jpeg(paste0( "Figures/04_SWP_difference_Season.jpg"))
partialPlot(rf, rf.data.val, Season)
dev.off()

jpeg(paste0( "Figures/04_SWP_difference_Distance.jpg"))
partialPlot(rf, rf.data.val, DistToSJC002)
dev.off()

jpeg(paste0( "Figures/04_SWP_difference_Q.jpg"))
partialPlot(rf, rf.data.val, Q)
dev.off()
jpeg(paste0( "Figures/04_SWP_difference_Species.jpg"))
partialPlot(rf, rf.data.val, Species)
dev.off()
jpeg(paste0( "Figures/04_SWP_difference_SJC.jpg"))
partialPlot(rf, rf.data.val, SJC_002)
dev.off()
jpeg(paste0( "Figures/04_SWP_difference_Replacement.jpg"))
partialPlot(rf, rf.data.val, Replacement)
dev.off()
jpeg(paste0( "Figures/04_SWP_difference_Group.jpg"))
partialPlot(rf, rf.data.val, Group)
dev.off()

jpeg(paste0( "Figures/04_SWP_difference_POS.jpg"))
partialPlot(rf, rf.data.val, POS)
dev.off()

jpeg(paste0( "Figures/04_SWP_difference_Substrate.jpg"))
partialPlot(rf, rf.data.val, Substrate)
dev.off()

jpeg(paste0( "Figures/04_SWP_difference_Year.jpg"))
partialPlot(rf, rf.data.val, Year)
dev.off()

jpeg(paste0( "Figures/04_SWP_difference_Damage.jpg"))
...
## all combined
mod1 <- lmer(formula = SWP.difference ~  Q_sc +  Replacement + Species + Group + 
...
icc(mod1)
r2_nakagawa(mod1) ## 0.495

## pom and sjc combined
mod2 <- lmer(formula = SWP.difference ~  Q_sc +  Replacement + Species + Group + 
...
icc(mod2)
r2_nakagawa(mod2) ## 0.495

## individual outfall discharge

mod3 <- lmer(formula = SWP.difference ~  Q_sc +  Replacement + Species + Group + 
...
ests
file.name1 <- paste0(out.dir, "effect_sizes_drivers_of_SWP_difference_inc_damage.jpg")
...
out.filename <- paste0(out.dir,"00_mixed_mod_SWP_difference_random_effects_inc_damage.jpg")
